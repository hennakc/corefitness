...
import { Award, Star, Dumbbell, Shield, ChevronLeft, ChevronRight } from "lucide-react";
...
// Slideshow photos — add your actual trainer images to /public/images/
const trainerPhotos = [
	{ src: "/images/trainer1.jpg", label: "In Action" },
	{ src: "/images/trainer2.jpg", label: "Training Session" },
	{ src: "/images/trainer3.jpg", label: "Competition" },
	{ src: "/images/trainer4.jpg", label: "Coaching" },
];

function TrainerSlideshow() {
	...
	const prev = () => {
		const idx = (current - 1 + trainerPhotos.length) % trainerPhotos.length;
		goTo(idx, "left");
	};

	const next = () => {
		const idx = (current + 1) % trainerPhotos.length;
		goTo(idx, "right");
	};
		...
		<div className="relative w-full mt-8">
			...
			>
				...
				<div className="absolute inset-0 rounded-2xl z-10 pointer-events-none"
					style={{
						boxShadow: "inset 0 0 60px 10px rgba(220,38,38,0.18), 0 0 80px 20px rgba(220,38,38,0.12)",
					}}
				/>
				...
				>
					<Image
						...
						alt={`Trainer - ${trainerPhotos[current].label}`}
						fill
						className="object-cover object-center"
						sizes="(max-width: 768px) 100vw, 50vw"
					/>
					{/* Bottom label */}
					<div className="absolute bottom-0 left-0 right-0 bg-gradient-to-t from-black/80 to-transparent pt-12 pb-4 px-5 z-10">
						<span className="text-white/70 text-xs tracking-widest uppercase">
							{trainerPhotos[current].label}
						</span>
					</div>
				</div>

				{/* Red top accent line */}
				<div className="absolute top-0 left-0 right-0 h-0.5 bg-gradient-to-r from-transparent via-brand-red to-transparent z-20" />
			</div>
			...
			<div className="flex items-center justify-between mt-4 px-1">
				...
				<div className="flex gap-2">
					<button
						onClick={prev}
						className="w-9 h-9 rounded-full border border-white/10 bg-white/5 flex items-center justify-center hover:bg-brand-red/20 hover:border-brand-red/40 transition-all duration-200"
					>
